...
import { WorkoutEffort } from '@/domain/types';
...
export default function CompleteScreen() {
  ...
  const level = getLevel(session?.level ?? 1);
  ...
  return <Screen contentStyle={styles.screen}>
    <View><View style={styles.hero}><View style={styles.icon}><Ionicons name="checkmark" size={42} color="#FFF" /></View><Text style={styles.title}>오늘 운동 완료</Text><Text style={styles.subtitle}>정해진 루틴을 모두 마쳤습니다.</Text></View>
      <View style={styles.results}>{session?.exercises.map((result) => { const exercise = level.exercises.find((item) => item.id === result.exerciseId); return <View key={result.exerciseId} style={styles.row}><Ionicons name="checkmark-circle" size={22} color={colors.primary} /><Text style={styles.name}>{exercise?.name ?? result.exerciseId}</Text><Text style={styles.count}>{result.completedSets}/{result.totalSets}세트</Text></View>; })}</View>
